    ...
    <div class="container">
        <header class="header catalog-header">
            <div class="head catalog-head">
                ...
                <nav>
                    ...
                    <a href="#" class="nav-active">Reviews</a>
                </nav>
        ...
        <section class="reviews">
            <div class="content">
                <h1>Reviews</h1>
                <?php
                $comments = comments_all($connect);
                if($comments){
                    foreach ($comments as $comment){
                        echo "<div style='border: 1px solid #ccc; margin: 10px; padding: 5px;;'>ФИО: {$comment["fio"]}<br>Email: {$comment["email"]}<br>Текст: {$comment["text"]}<br></div>";
                    }
                }
                ?>
                <hr>
                <form method="post" action="#">
                    ...
                    <p>Write your name: <input type="text" name="fio" maxlength="30" required></p>
                    <p>Write Email: <input type="email" name="email" maxlength="20" required></p>
                    <p>Write text: <textarea name="text" rows="10" required></textarea></p>
